...
function DistributionBar({ label, value, total, color }) {
  ...
  return (
    <div
      className="audit-dashboard-bar"
      title={`${label}: ${value} de ${total || 0} (${pct}%)`}
    >
...
function labelSlotWidth(labels, min = 124, max = 190) {
  const biggest = labels.reduce(
    (size, label) => Math.max(size, String(label || "").length),
    0,
  );
  return Math.max(min, Math.min(max, biggest * 7));
}

function SvgAxisLabel({
  x,
  y,
  label,
  maxChars = 14,
  maxLines = 3,
  className = "audit-dashboard-combo__label",
}) {
  ...
  return (
    <div className={className}>
      <svg viewBox="0 0 100 100" aria-hidden="true">
        <circle
          className="audit-dashboard-donut__track"
          cx="50"
          cy="50"
          r={radius}
        />
  ...
  return Array.from(gruposMap.values())
    .map((item) => {
      ...
      return {
        ...item,
        avaliados,
        maturidade: avaliados
          ? Math.round((item.conformes / avaliados) * 100)
          : 0,
      };
    })
...
function calcularMetricas(controles) {
  ...
  const maturidade = controlesAvaliados
    ? Math.round((conformes / controlesAvaliados) * 100)
    : 0;
  ...
  return {
    ...
    porPropriedade: buildMaturidadePorGrupo(controles, (item) =>
      getControleTags(item, "propriedades_si", "Sem propriedade definida"),
    ),
    porConceito: buildMaturidadePorGrupo(controles, (item) =>
      getControleTags(item, "conceitos_si", "Sem conceito definido"),
    ),
    porCapacidade: buildMaturidadePorGrupo(controles, (item) =>
      getControleTags(
        item,
    ...
    porDominio: buildMaturidadePorGrupo(controles, (item) =>
      getControleTags(item, "dominios_seguranca", "Sem domínio definido"),
    ),
    porTema: buildMaturidadePorGrupo(controles, (item) =>
      ...
      <svg viewBox="0 0 236 236" role="img" aria-label="Radar de conformidade">
        ...
        <polygon points={polygon} className="audit-dashboard-radar__shape">
          <title>
            ...
            {chartItems
              .map((item) => `${item.label} ${item.maturidade}%`)
              .join(", ")}
          </title>
        </polygon>
          ...
          return (
            <g key={item.label}>
              <circle
                ...
                cy={point.y}
                r="3.8"
                className="audit-dashboard-radar__dot"
              >
                <title>
                  {item.label}: {item.maturidade}%
                </title>
              </circle>
              <SvgAxisLabel
                x={label.labelX}
    ...
    return (
      <div className="audit-dashboard-radar-fallback">
        {chartItems.map((item) => (
          <div
            key={item.label}
            title={`${item.label}: atual ${item.atual}% / histórico ${item.historico}%`}
          >
            <span>{item.label}</span>
            <strong>
              {item.atual}% · {item.historico}%
            </strong>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="audit-dashboard-radar audit-dashboard-radar--comparative">
      <svg
        viewBox="0 0 236 236"
        role="img"
        aria-label="Radar comparativo de maturidade"
      >
        {rings.map((ring) => (
          <polygon
            key={ring}
        ...
        <polygon
          points={polygonFor("historico")}
          className="audit-dashboard-radar__shape audit-dashboard-radar__shape--history"
        >
          <title>
            Histórico médio:{" "}
            {chartItems
              .map((item) => `${item.label} ${item.historico}%`)
              .join(", ")}
          </title>
        </polygon>
        <polygon
          points={polygonFor("atual")}
          className="audit-dashboard-radar__shape"
        >
          <title>
            Atual:{" "}
            {chartItems
              .map((item) => `${item.label} ${item.atual}%`)
              .join(", ")}
          </title>
        </polygon>
        {chartItems.map((item, index) => {
          ...
          return (
            <g key={item.label}>
              ...
              >
                <title>
                  {item.label}: atual {item.atual}% / histórico {item.historico}
                  %
                </title>
              </circle>
              <SvgAxisLabel
    ...
    <div className="audit-dashboard-group-pie">
      ...
      <div className="audit-dashboard-group-pie__legend">
        {chartItems.map((item, index) => (
          <div
            key={item.label}
            title={`${item.label}: ${item.total} controles`}
          >
...
function ComboComplianceChart({ items }) {
  const chartItems = items.slice(0, 8);
  const slotMinWidth = labelSlotWidth(
  ...
  const maxTotal = Math.max(
    1,
    ...chartItems.map((item) => item.avaliados || item.total || 0),
  );
  ...
  return (
    <div className="audit-dashboard-combo">
      <div className="audit-dashboard-combo__headline">
        <strong>
          {Math.round(
            chartItems.reduce((sum, item) => sum + item.maturidade, 0) /
              chartItems.length,
          )}
          %
        </strong>
      ...
      <div className="audit-dashboard-combo__scroll">
        <div className="audit-dashboard-combo__canvas" style={{ width }}>
          <svg
            ...
            role="img"
            aria-label="Controles avaliados e maturidade"
          >
              ...
              return (
                <g key={tick}>
                  <line
                    ...
                    y1={y}
                    y2={y}
                    className="audit-dashboard-combo__grid"
                  />
                  <text
                    x={padding.left - 10}
                    y={y + 4}
                    textAnchor="end"
                    className="audit-dashboard-combo__axis"
                  >
                    {tick}%
                  </text>
                </g>
            ...
            {chartItems.map((item, index) => {
              const x = xFor(index);
              const avaliadosHeight =
                padding.top + innerHeight - yForCount(item.avaliados);
              const conformesHeight =
                padding.top + innerHeight - yForCount(item.conformes);
              ...
              return (
                <g key={item.label}>
                  ...
                  >
                    <title>
                      {item.label}: {item.avaliados} avaliados
                    </title>
                  </rect>
                  <rect
                  ...
                  >
                    <title>
                      {item.label}: {item.conformes} conformes
                    </title>
                  </rect>
                </g>
              );
            ...
            <polyline
              points={linePoints}
              className="audit-dashboard-combo__line"
            >
              <title>
                Maturidade:{" "}
                {chartItems
            ...
            {chartItems.map((item, index) => (
              ...
              >
                <title>
                  {item.label}: {item.maturidade}% de maturidade
                </title>
              </circle>
            ))}
          </svg>
          ...
          >
            {chartItems.map((item) => (
              <span key={`label-${item.label}`} title={item.label}>
                {item.label}
              </span>
            ))}
          </div>
      ...
      <div className="audit-dashboard-combo__legend">
        <span>
          <i className="audit-dashboard-combo__legend-total" /> Avaliados
        </span>
        <span>
          <i className="audit-dashboard-combo__legend-ok" /> Conformes
        </span>
        <span>
          <i className="audit-dashboard-combo__legend-line" /> Maturidade
        </span>
      </div>
    </div>
  );
        ...
        <div className="audit-dashboard-insights">
          ...
          <div className="audit-dashboard-insights__cards">
            {rankedItems.map((item) => (
              <article
                key={item.label}
                className="audit-dashboard-insight-card"
              >
                <div>
                ...
                <em style={{ color: maturityTone(item.maturidade) }}>
                  {item.maturidade}%
                </em>
              </article>
            ))}
  ...
  return (
    <section className="audit-dashboard-panel">
      <div
        className={
          actions
            ? "audit-dashboard-panel__title audit-dashboard-panel__title--split"
            : "audit-dashboard-panel__title"
        }
      >
        <div>
          <Layers3 size={18} />
      ...
      {items.length === 0 ? (
        ...
        <div
          className={`audit-dashboard-group audit-dashboard-group--${variant}`}
        >
          {variant === "combo" && (
            <ComboComplianceChart items={featuredItems} />
          )}
          {variant === "radar" && <RadarChart items={featuredItems} />}
          {variant === "vertical" && (
            <VerticalBarsChart items={featuredItems} />
          )}
            ...
            {featuredItems.map((item) => (
              <div key={item.label} className="audit-dashboard-type-row">
                ...
                <div className="audit-dashboard-type-row__chart">
                  <div
                    className="audit-dashboard-bar__track"
                    title={`${item.label}: ${item.maturidade}%`}
                  >
                    <div
                      style={{
                        width: `${item.maturidade}%`,
                        backgroundColor: maturityTone(item.maturidade),
                      }}
                    />
                  </div>
    ...
    <div className="audit-dashboard-pizza-wrap">
      ...
      <div className="audit-dashboard-legend">
        {entries.map(([key, value]) => (
          <div
            key={key}
            title={`${statusConfig[key].label}: ${value} de ${metricas.total || 0}`}
          >
            ...
            <strong>
              ...
              <small>
                {metricas.total
                  ? Math.round((value / metricas.total) * 100)
                  : 0}
                %
              </small>
            </strong>
...
function ComparisonDashboard({
  ...
  const [grupoComparativo, setGrupoComparativo] = useState(
    isNorma27701 ? "categoria" : "tipos_controle",
  );
  const comparisonSlotWidth = labelSlotWidth(
    chartItems.map((item) => auditLabel(item.auditoria)),
    136,
    220,
  );
  ...
  const yFor = (value) =>
    padding.top + innerHeight - (value / 100) * innerHeight;
  const maxConformes = Math.max(
    1,
    ...chartItems.map((item) => item.metricas.conformes),
  );
  const yForConformes = (value) =>
    padding.top + innerHeight - (value / maxConformes) * innerHeight;
  const maturidadePoints = chartItems
    .map((item, index) => `${xFor(index)},${yFor(item.metricas.maturidade)}`)
    .join(" ");
  const progressoPoints = chartItems
    .map((item, index) => `${xFor(index)},${yFor(item.metricas.progresso)}`)
    .join(" ");
  const conformesPoints = chartItems
    .map(
      (item, index) =>
        `${xFor(index)},${yForConformes(item.metricas.conformes)}`,
    )
    .join(" ");
  const auditoriaAtual = chartItems.find((item) => item.atual) || chartItems[0];
  const comparisonItemKey = (item, index) => {
    const auditoriaId =
      item.auditoria?.id_auditoria ?? item.auditoria?.id ?? "sem-id";
    ...
    ? [
        {
          ...
          label: "Categoria",
          field: "porCategoria",
          empty: "Nenhuma categoria encontrada para comparar.",
        },
        {
          key: "atributos_anexo",
          label: "atributo_anexo",
          field: "porAtributo",
          empty: "Nenhum atributo_anexo encontrado para comparar.",
        },
        {
          key: "tipos_controle",
          label: "tipo_controle",
          field: "porTipo",
          empty: "Nenhum tipo_controle encontrado para comparar.",
        },
      ]
    : [
        {
        ...
        },
        {
          key: "propriedade",
          label: "Propriedade",
          field: "porPropriedade",
          empty: "Nenhuma propriedade encontrada para comparar.",
        },
        {
          key: "conceito",
          label: "Conceito",
          field: "porConceito",
          empty: "Nenhum conceito encontrado para comparar.",
        },
        {
          key: "capacidade",
          label: "Capacidade operacional",
          field: "porCapacidade",
          empty: "Nenhuma capacidade operacional encontrada para comparar.",
        },
        {
          key: "tema",
          label: "Tema",
          field: "porTema",
          empty: "Nenhum tema encontrado para comparar.",
        },
        {
          key: "dominio",
          label: "Domínio",
          field: "porDominio",
          empty: "Nenhum domínio encontrado para comparar.",
        },
      ];
  const grupoAtual =
    gruposDisponiveis.find((grupo) => grupo.key === grupoComparativo) ||
    gruposDisponiveis[0];
  const itensGrupoAtual = (
    auditoriaAtual?.metricas[grupoAtual.field] || []
  ).slice(0, 8);
  const categorias = itensGrupoAtual.map((item) => item.label);
  const itensComparativos = categorias.map((categoria) => {
    const atual =
      auditoriaAtual?.metricas[grupoAtual.field]?.find(
    ...
    const historicos = chartItems
      ...
      .map(
        (item) =>
          item.metricas[grupoAtual.field].find(
            (valor) => valor.label === categoria,
          )?.maturidade || 0,
      );
    const historico = historicos.length
      ? Math.round(
          historicos.reduce((sum, value) => sum + value, 0) / historicos.length,
        )
      : 0;
  ...
  return (
    <section className="audit-dashboard-panel audit-dashboard-comparison audit-dashboard-comparison--compact">
      <div className="audit-dashboard-panel__title audit-dashboard-panel__title--split">
        ...
        <div
          className="audit-dashboard-tabs"
          role="tablist"
          aria-label="Quantidade de auditorias comparadas"
        >
          {[
            ["3", "Últimas 3"],
            ["5", "Últimas 5"],
            ...
            <button
              ...
              className={
                range === value
                  ? "audit-dashboard-tabs__item audit-dashboard-tabs__item--active"
                  : "audit-dashboard-tabs__item"
              }
      ...
      {loading ? (
        ...
        <>
          <div className="audit-dashboard-comparison__charts">
            <div className="audit-dashboard-comparison__chart-card">
              ...
              <div className="audit-dashboard-comparison__chart">
                <div
                  className="audit-dashboard-comparison__canvas"
                  style={{ width }}
                >
                  <svg
                    viewBox={`0 0 ${width} ${height}`}
                    role="img"
                    aria-label="Comparativo de maturidade e progresso"
                  >
                    {[0, 25, 50, 75, 100].map((tick) => {
                      const y = yFor(tick);
                      return (
                    ...
                    <polyline
                      points={progressoPoints}
                      className="audit-dashboard-comparison__line audit-dashboard-comparison__line--progress"
                    >
                      <title>
                        Progresso:{" "}
                        {chartItems
                          .map(
                            (item) =>
                              `${auditLabel(item.auditoria)} ${item.metricas.progresso}%`,
                          )
                          .join(", ")}
                      </title>
                    </polyline>
                    <polyline
                      points={maturidadePoints}
                      className="audit-dashboard-comparison__line"
                    >
                      <title>
                        Maturidade:{" "}
                        {chartItems
                          .map(
                            (item) =>
                              `${auditLabel(item.auditoria)} ${item.metricas.maturidade}%`,
                          )
                          .join(", ")}
                      </title>
                    </polyline>
                    {chartItems.map((item, index) => (
                      <g key={`maturidade-${comparisonItemKey(item, index)}`}>
                        ...
                        >
                          <title>
                            {auditLabel(item.auditoria)}: maturidade{" "}
                            {item.metricas.maturidade}% / progresso{" "}
                            {item.metricas.progresso}%
                          </title>
                        </circle>
                      </g>
                    ))}
                  ...
                  >
                    {chartItems.map((item, index) => (
                      <span
                        key={`maturity-label-${comparisonItemKey(item, index)}`}
                        title={auditLabel(item.auditoria)}
                      >
              ...
              <div className="audit-dashboard-comparison__chart">
                ...
                >
                  <svg
                    viewBox={`0 0 ${width} ${height}`}
                    role="img"
                    aria-label="Comparativo de conformes"
                  >
                    ...
                    {chartItems.map((item, index) => {
                      ...
                      const barHeight =
                        padding.top +
                        innerHeight -
                        yForConformes(item.metricas.conformes);
                      return (
                        <g key={`conformes-${comparisonItemKey(item, index)}`}>
                          <rect
                            ...
                            className={
                              item.atual
                                ? "audit-dashboard-comparison__bar audit-dashboard-comparison__bar--current"
                                : "audit-dashboard-comparison__bar"
                            }
                          >
                            <title>
                              {auditLabel(item.auditoria)}:{" "}
                              {item.metricas.conformes} conformes
                            </title>
                          </rect>
                        </g>
                      );
                    ...
                    <polyline
                      points={conformesPoints}
                      className="audit-dashboard-comparison__line audit-dashboard-comparison__line--conformes"
                    >
                      <title>
                        Conformes:{" "}
                        {chartItems
                          .map(
                            (item) =>
                              `${auditLabel(item.auditoria)} ${item.metricas.conformes}`,
                          )
                          .join(", ")}
                      </title>
                    </polyline>
                  ...
                  >
                    {chartItems.map((item, index) => (
                      <span
                        key={`conformes-label-${comparisonItemKey(item, index)}`}
                        title={auditLabel(item.auditoria)}
                      >
                        {auditLabel(item.auditoria)}
          ...
          <div className="audit-dashboard-comparison__legend">
            <span>
              <i className="audit-dashboard-comparison__legend-current" />{" "}
              Auditoria atual
            </span>
            <span>
              <i className="audit-dashboard-comparison__legend-history" />{" "}
              Histórico
            </span>
            <span>
              <i className="audit-dashboard-comparison__legend-line" />{" "}
              Maturidade
            </span>
            <span>
              <i className="audit-dashboard-comparison__legend-progress" />{" "}
              Progresso
            </span>
            <span>
              <i className="audit-dashboard-comparison__legend-conformes" />{" "}
              Conformes
            </span>
          </div>
          ...
          <div className="audit-dashboard-comparison__cards">
            {chartItems.map((item, index) => (
              <article
                ...
                className={
                  item.atual
                    ? "audit-dashboard-comparison-card audit-dashboard-comparison-card--current"
                    : "audit-dashboard-comparison-card"
                }
                title={`${auditLabel(item.auditoria)}: ${item.metricas.maturidade}% maturidade, ${item.metricas.progresso}% progresso`}
              >
                <p>
                  {item.atual ? "Auditoria atual" : "Histórico"} ·{" "}
                  {auditLabel(item.auditoria)}
                </p>
                <strong>{item.metricas.maturidade}%</strong>
                <span>
                  {item.metricas.conformes} conforme /{" "}
                  {item.metricas.controlesAvaliados} avaliados
                </span>
          ...
          <div className="audit-dashboard-comparison-category">
            ...
            <div
              className="audit-dashboard-tabs audit-dashboard-tabs--compact"
              role="tablist"
              aria-label="Agrupamento do comparativo"
            >
              ...
              <>
                <div className="audit-dashboard-comparison-insights">
                  ...
                  <div className="audit-dashboard-comparison-rank">
                    ...
                    {itensComparativos.map((item) => (
                      <article
                        key={item.label}
                        title={`${item.label}: atual ${item.atual}% / histórico ${item.historico}% / variação ${item.delta >= 0 ? "+" : ""}${item.delta}%`}
                      >
                        <span>{item.label}</span>
                ...
                <div className="audit-dashboard-comparison-category__grid">
                  {categorias.map((categoria) => (
                    <article
                      key={categoria}
                      className="audit-dashboard-category-mini"
                    >
                      <strong>{categoria}</strong>
                      <div className="audit-dashboard-category-mini__bars">
                        {chartItems.map((item) => {
                          const categoriaMetricas = item.metricas[
                            grupoAtual.field
                          ].find((valor) => valor.label === categoria);
                          const maturidade = categoriaMetricas?.maturidade || 0;
                          return (
                            <div
                              key={`${categoria}-${comparisonItemKey(item, 0)}`}
                            >
                              <span
                                title={`${auditLabel(item.auditoria)}: ${maturidade}%`}
                                className={
                                  item.atual
                                    ? "audit-dashboard-category-mini__bar audit-dashboard-category-mini__bar--current"
                                    : "audit-dashboard-category-mini__bar"
                                }
                                style={{
                                  height: `${Math.max(maturidade, 4)}%`,
                                }}
                              />
...
export default function DashboardAuditoriaPage() {
  ...
  const carregarRespostasAuditoria = async (idAuditoria, norma) => {
    ...
    const respostasResp = await fetch(
      `${BASE}/auditoria/${idAuditoria}/${normaSlug}/respostas/`,
      {
    ...
    try {
      ...
      if (!contexto.norma) {
        ...
        if (listaResp.ok) {
          ...
          contexto =
            lista.find(
              (a) => String(a.id_auditoria) === String(id_auditoria),
            ) || {};
      ...
      setControles(
        await carregarRespostasAuditoria(id_auditoria, contexto.norma),
      );
    } catch {
  ...
  const agrupamentoAtual =
    abasAgrupamento.find((aba) => aba.key === abaAgrupamento) ||
    abasAgrupamento[0];
  ...
  useEffect(() => {
    ...
    const carregarComparativo = async () => {
      ...
      try {
        ...
        const lista = await listaResp.json();
        const auditoriaDaLista = lista.find(
          (item) => String(item.id_auditoria) === String(id_auditoria),
        );
        const auditoriaBase = {
          ...(auditoriaDaLista || {}),
          ...auditoria,
          ...
          data_auditoria:
            auditoria.data_auditoria ?? auditoriaDaLista?.data_auditoria,
        };
        ...
        const selecionadas =
          comparativoRange === "all"
            ? mesmaEmpresaNorma
            : mesmaEmpresaNorma.slice(0, Number(comparativoRange));

        const historico = await Promise.all(
          selecionadas.map(async (item) => {
            const respostas = await carregarRespostasAuditoria(
              item.id_auditoria,
              item.norma,
            );
            return {
        ...
        setComparativoErro(
          "Não foi possível carregar o comparativo desta empresa e norma.",
        );
      } finally {
  ...
  return (
    <div className="audit-dashboard-page">
      ...
      <main className="audit-dashboard-page__main">
        ...
        <div
          className="audit-dashboard-view-tabs"
          role="tablist"
        ...
        >
          <button
            ...
            className={
              abaDashboard === "atual"
                ? "audit-dashboard-view-tabs__item audit-dashboard-view-tabs__item--active"
                : "audit-dashboard-view-tabs__item"
            }
          ...
          <>
            <section className="audit-dashboard-summary">
              <div>
                ...
                <span>
                  Maturidade = {metricas.conformes} conforme /{" "}
                  {metricas.controlesAvaliados} controles avaliados
                </span>
              </div>
              <div
                className="audit-dashboard-ring"
                style={{ "--pct": `${metricas.progresso}%` }}
              >
            ...
            <section className="audit-dashboard-metrics">
              <MetricCard
                icon={ClipboardList}
                label="Total"
                value={loading ? "..." : metricas.total}
                helper="Controles avaliáveis"
                color="#6B0F2B"
              />
              <MetricCard
                icon={ShieldCheck}
                label="Avaliados"
                value={loading ? "..." : metricas.controlesAvaliados}
                helper="Total menos não se aplica"
                color="#6B0F2B"
              />
              <MetricCard
                icon={CheckCircle2}
                label="Conformes"
                value={loading ? "..." : metricas.conformes}
                helper="Atendem ao controle"
                color="#14532d"
              />
              <MetricCard
                icon={XCircle}
                label="Não Conformes"
                value={loading ? "..." : metricas.naoConformes}
              ...
              <MetricCard
                icon={MinusCircle}
                label="Não Aplica"
                value={loading ? "..." : metricas.naoAplica}
                helper="Fora do escopo"
                color="#78350f"
              />
              <MetricCard
                icon={TimerReset}
                label="Pendentes"
                value={loading ? "..." : metricas.pendentes}
                helper="Sem resposta"
                color="#6B0F2B"
              />
            </section>
