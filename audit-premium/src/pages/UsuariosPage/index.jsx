...
export default function UsuariosPage() {
  ...
  const atualizarUsuario = async (usuario, patch) => {
    ...
    try {
      ...
      if (!r.ok) {
        setErro(data.detail || "Erro ao atualizar usuário.");
        return;
      }
  ...
  const criarCodigo = async (e) => {
    ...
    try {
      ...
      const data = await r.json().catch(() => ({}));
      if (!r.ok) {
        setErro(data.detail || "Erro ao criar código.");
        return;
      }
  ...
  const atualizarCodigo = async (codigo, patch) => {
    setErro("");
    setSucesso("");
    setSalvandoCodigo(codigo.id_codigo);
    try {
      const r = await fetch(
        `${API}/admin/codigos-acesso/${codigo.id_codigo}/`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
      ...
      const data = await r.json().catch(() => ({}));
      if (!r.ok) {
        setErro(data.detail || "Erro ao atualizar código.");
        return;
      }
      setCodigos((prev) =>
        prev.map((c) => (c.id_codigo === codigo.id_codigo ? data : c)),
      );
  ...
  const importarCSV = async (e) => {
    ...
    try {
      ...
      const data = await r.json().catch(() => ({}));
      if (!r.ok) {
        setErro(data.detail || "Erro ao importar CSV.");
        return;
      }
      setSucesso(
        `${data.criados || 0} criados, ${data.atualizados || 0} atualizados.`,
      );
      fetchCodigos();
    } catch {
      setErro("Erro ao conectar com o servidor.");
  ...
  return (
    <div
      className="usuarios-page app-shell-page"
      style={{
        minHeight: "100vh",
    ...
    >
                  ...
                  >
                    ...
                    >
                      ...
                      <button
                        ...
                        style={{
                          ...buttonStyle,
                          opacity: salvandoUsuario === usuario.id ? 0.6 : 1,
                        }}
                      >
                        <Save size={13} /> Salvar
                      </button>
              ...
              >
                  ...
                  >
                    <input
                      ...
                      onChange={(e) =>
                        setNovoCodigo((p) => ({
                          ...p,
                          codigo: e.target.value.toUpperCase().slice(0, 15),
                        }))
                      }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNovoCodigo((p) => ({ ...p, nome: e.target.value }))
                    }
                    placeholder="Nome permitido"
                    style={inputStyle}
                  />
                </div>
                <div>
                  ...
                  <input
                    type="email"
                    value={novoCodigo.email}
                    onChange={(e) =>
                      setNovoCodigo((p) => ({ ...p, email: e.target.value }))
                    }
                ...
                <div>
                  <p style={labelStyle}>Cargo</p>
                  <input
                    value={novoCodigo.cargo}
                    onChange={(e) =>
                      setNovoCodigo((p) => ({ ...p, cargo: e.target.value }))
                    }
                    style={inputStyle}
                  />
                </div>
                ...
                {codigos.map((codigo) => (
                  ...
                  >
                    <div>
                      ...
                      >
                        {codigo.usado_por_username
                          ? `Usado por ${codigo.usado_por_username}`
                          : "Disponível"}
                      </p>
                    </div>
                    <input
                    ...
                    >
                      <button
                        type="button"
                        onClick={() =>
                          atualizarCodigo(codigo, { ativo: !codigo.ativo })
                        }
                        style={{
                          ...buttonStyle,
                          backgroundColor: "transparent",
                      ...
                      <button
                        type="button"
                        disabled={salvandoCodigo === codigo.id_codigo}
                        onClick={() =>
                        ...
                        style={{
                          ...buttonStyle,
                          opacity:
                            salvandoCodigo === codigo.id_codigo ? 0.6 : 1,
                        }}
                      >
                        <Save size={13} /> Salvar
                      </button>
      ...
      {usuarioAtualModal && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) fecharUsuarioModal();
          }}
          style={{
            position: "fixed",
            inset: 0,
            ...
            >
              <div style={{ display: "flex", gap: 18, alignItems: "center" }}>
                ...
                <div>
                  ...
                  >
                    {`${usuarioModalForm.first_name} ${usuarioModalForm.last_name}`.trim() ||
                      usuarioAtualModal.name}
                  </h2>
            ...
            <div style={{ overflowY: "auto", padding: "26px 34px 34px" }}>
              ...
              >
                ...
                >
                  ...
                  <input
                    value={usuarioModalForm.first_name}
                    onChange={(e) =>
                      setUsuarioModalForm((p) => ({
                        ...p,
                ...
                >
                  ...
                  <input
                    value={usuarioModalForm.last_name}
                    onChange={(e) =>
                      setUsuarioModalForm((p) => ({
                        ...p,
                ...
                <DetailItem
                  ...
                  value={`@${usuarioAtualModal.username}`}
                />
                <div
                  style={{
                    padding: "14px 0",
                ...
                <DetailItem
                  icon={Calendar}
                  label="Criado em"
                  value={formatarData(usuarioAtualModal.date_joined)}
                />
                <DetailItem
                  icon={ShieldCheck}
                  label="Tipo de conta"
              ...
              >
                ...
                <button
                  ...
                  style={{
                    ...buttonStyle,
                    opacity: salvandoUsuario === usuarioAtualModal.id ? 0.6 : 1,
                  }}
                >
                  <Save size={13} />{" "}
                  {salvandoUsuario === usuarioAtualModal.id
                    ? "Salvando..."
                    : "Salvar Alterações"}
                </button>
              </div>
