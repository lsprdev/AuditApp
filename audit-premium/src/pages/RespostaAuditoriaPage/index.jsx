...
import { controlesUrl, mediaUrl } from "../../config/api";
import "./styles.css";
...
const STATUS_MAP = {
  CONFORME: {
    label: "Conforme",
    color: "#14532d",
    bg: "rgba(20,83,45,0.09)",
    Icon: CheckCircle2,
  },
  NAO_CONFORME: {
    label: "Não Conforme",
    color: "#7f1d1d",
    bg: "rgba(127,29,29,0.09)",
    Icon: XCircle,
  },
  NAO_APLICA: {
    label: "Não Aplica",
...
const BASE_AUDITORIAS = controlesUrl("/auditorias");
...
function ControleRow({
  ...
  const handleStatusClick = async (e, statusKey) => {
    ...
    try {
      ...
      if (resp.ok) {
        onStatusChange({
          ...item,
          resposta: { ...item.resposta, situacao: statusKey },
        });
      }
    } catch {
    } finally {
  ...
  return (
    <div
      ...
      onMouseEnter={(e) =>
        (e.currentTarget.style.boxShadow = "0 4px 16px rgba(107,15,43,0.08)")
      }
    ...
    >
      ...
      <div
        className="resposta-quick-answer"
        style={{
          display: "flex",
          alignItems: "center",
          gap: 6,
      ...
      >
        <span
          className="resposta-quick-answer__label"
          style={{
            fontFamily: "var(--font-sans)",
            fontSize: 11,
        ...
        {Object.entries(STATUS_MAP).map(([key, { label, Icon, color, bg }]) => {
          ...
          return (
            <button
              className="resposta-quick-answer__button"
              key={key}
              ...
              style={{
                ...
                padding: "4px 12px",
                borderRadius: 6,
                cursor: bloqueado || saving ? "not-allowed" : "pointer",
                fontFamily: "var(--font-sans)",
                fontSize: 11,
                letterSpacing: "0.1em",
                textTransform: "uppercase",
                ...
                border: isSelected
                  ...
                  : "1.5px solid rgba(107,15,43,0.12)",
                opacity:
                  bloqueado && !isSelected
                    ? 0.45
                    : saving && !isLoading
                      ? 0.5
                      : 1,
              ...
              onMouseEnter={(e) => {
                if (!isSelected && !saving && !bloqueado) {
              ...
              onMouseLeave={(e) => {
                if (!isSelected) {
                  ...
                  e.currentTarget.style.borderColor = "rgba(107,15,43,0.12)";
                }
              }}
            >
              {isLoading ? (
        ...
        {items.map((t, i) => (
          <span
            ...
            style={{
              ...
              fontSize: 11,
              fontFamily: "var(--font-sans)",
              backgroundColor: "rgba(107,15,43,0.06)",
              color: "#6B0F2B",
...
function DetalheModal({
  ...
  const handleSalvar = async (e) => {
    e.preventDefault();
    if (bloqueado) {
      setErro("Auditoria concluída não permite alteração de respostas.");
      return;
    }
    if (!form.situacao) {
      setErro("Selecione uma situação.");
      return;
    }
    setErro("");
    setSucesso("");
  ...
  const handleDeletar = async () => {
    if (bloqueado) {
      setErro("Auditoria concluída não permite excluir respostas.");
      return;
    }
  ...
  return (
    <div
      ...
      style={{
        position: "fixed",
        inset: 0,
        backgroundColor: "rgba(20,5,12,0.5)",
        display: "flex",
        alignItems: "center",
        justifyContent: "center",
        zIndex: 200,
    ...
    >
      <div
        className="resposta-modal smooth-pop"
        style={{
      ...
      >
        {/* Header */}
        <div
          className="resposta-modal__header"
          style={{
              ...
              {form.situacao ? (
                ...
                <span
                  style={{
                    ...
                    padding: "5px 14px",
                    border: "1px dashed rgba(107,15,43,0.2)",
                    borderRadius: 6,
                  }}
                >
                  Pendente
        ...
        {/* Body */}
        <div
          className="resposta-modal__body"
          style={{
            overflowY: "auto",
            flex: 1,
        ...
        >
          {c.descricao && (
            <div>
              ...
              <p style={{ ...valueStyle, color: "rgba(61,8,24,0.75)" }}>
                {c.descricao}
              </p>
            </div>
          )}
          ...
          {(c.tipos_controle?.length > 0 ||
            c.propriedades_si?.length > 0 ||
            c.conceitos_si?.length > 0 ||
            c.capacidades_operacionais?.length > 0 ||
            c.dominios_seguranca?.length > 0 ||
            c.temas_controle?.length > 0) && (
            <div
              style={{
                display: "flex",
                flexDirection: "column",
                gap: 14,
                padding: 20,
                backgroundColor: "rgba(107,15,43,0.03)",
                borderRadius: "8px",
            ...
            >
              ...
              <TagList
                items={c.capacidades_operacionais}
                label="Capacidades Operacionais"
              />
              <TagList
                items={c.dominios_seguranca}
                label="Domínios de Segurança"
              />
              <TagList items={c.temas_controle} label="Temas de Controle" />
            </div>
          )}

          {(c.atributos_anexos?.length > 0 ||
            c.tipos_controle_anexo?.length > 0) && (
            <div
              style={{
                display: "flex",
                flexDirection: "column",
            ...
            >
              {c.anexoB && (
                <div>
                  <span style={labelStyle}>Anexo B</span>
                  <p style={valueStyle}>{c.anexoB}</p>
                </div>
              )}
              {c.orientacao_anexoB && (
                <div>
                  <span style={labelStyle}>Orientação Anexo B</span>
                  <p style={{ ...valueStyle, color: "rgba(61,8,24,0.75)" }}>
                    {c.orientacao_anexoB}
                  </p>
                </div>
              ...
              <TagList
                items={c.tipos_controle_anexo}
                label="Tipos de Controle Anexo"
              />
            </div>
          )}
          ...
          <div>
            <div
              className="resposta-modal__answer-header"
              style={{
                display: "flex",
                alignItems: "center",
                justifyContent: "space-between",
            ...
            >
              ...
              {!modoEdicao && temResposta && !bloqueado && (
                <div
                  className="resposta-modal__answer-actions"
            ...
            {!modoEdicao && temResposta && (
              ...
              >
                ...
                {r.possui_andamento !== null &&
                  r.possui_andamento !== undefined && (
                    <div>
                      <span style={labelStyle}>Possui Andamento</span>
                      <p style={valueStyle}>
                        {r.possui_andamento ? "Sim" : "Não"}
                      </p>
                    </div>
                  )}
              </div>
              ...
              >
                <div>
                  ...
                  <div style={{ position: "relative" }}>
                    ...
                    >
                      {form.situacao
                        ? STATUS_MAP[form.situacao]?.label
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Justifique a situação escolhida..."
                    style={{ ...inputStyle, paddingTop: 8 }}
                  />
                </div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <button
                    ...
                    onClick={() =>
                      setForm((p) => ({
                        ...p,
                        possui_andamento: !p.possui_andamento,
                      }))
                    }
                    style={{
                      width: 40,
                      ...
                      border: "none",
                      cursor: "pointer",
                      backgroundColor: form.possui_andamento
                        ? "#6B0F2B"
                        : "rgba(107,15,43,0.15)",
                  ...
                  >
                    <div
                      style={{
                        ...
                        position: "absolute",
                        top: 3,
                        left: form.possui_andamento ? 21 : 3,
                ...
                <div
                  className="resposta-modal__form-actions"
                  style={{
                    display: "flex",
                    gap: 10,
                  ...
                  >
                    <input
                      ...
                      onChange={(e) =>
                        setForm((p) => ({
                          ...p,
                          evidFile: e.target.files?.[0] || null,
                        }))
                      }
                      style={{ flex: 1 }}
                    />
...
export default function RespostasAuditoriaPage() {
  ...
  const handleRespostaAtualizada = (itemAtualizado) => {
    setControles((prev) =>
      prev.map((c) =>
        c.controle.id === itemAtualizado.controle.id ? itemAtualizado : c,
      ),
    );
  ...
  const handleStatusChange = (itemAtualizado) => {
    ...
    if (modalItem?.controle?.id === itemAtualizado.controle.id)
      setModalItem(itemAtualizado);
  };
  ...
  const handleFinalizarAuditoria = async () => {
    ...
    if (
      !window.confirm(
        "Deseja finalizar esta auditoria? O status será alterado para Concluída.",
      )
    )
      return;
    setFinalizando(true);
  ...
  return (
    <div
      className="resposta-auditoria-page app-shell-page"
      style={{
        minHeight: "100vh",
              ...
              <>
                ...
                <span
                  style={{
                    ...
                    height: 3,
                    borderRadius: "50%",
                    backgroundColor: "rgba(107,15,43,0.25)",
                  }}
                />
              </>
            )}
          ...
          >
            <div
              style={{
                ...
                height: 40,
                borderRadius: "50%",
                border: "2px solid rgba(107,15,43,0.15)",
                borderTopColor: "#6B0F2B",
              ...
              {FILTROS.map((f) => (
                <button
                  ...
                  style={{
                    ...
                    color:
                      filtro === f.key ? "#D4C5A9" : "rgba(107,15,43,0.55)",
                    border:
                      filtro === f.key
                        ? "1px solid #6B0F2B"
                        : "1px solid rgba(107,15,43,0.15)",
                  }}
                >
                  {f.label}
            ...
            >
              {auditoriaFinalizada ? (
                ...
                <>
                  ...
                  >
                    {respondidos.length} de {controles.length} controles
                    respondidos
                  </p>
                  <button
                    ...
                    onMouseEnter={(e) => {
                      if (!finalizando)
                        e.currentTarget.style.boxShadow =
                          "0 6px 28px rgba(107,15,43,0.4)";
                    }}
                    onMouseLeave={(e) => {
                      e.currentTarget.style.boxShadow =
                        "0 4px 20px rgba(107,15,43,0.25)";
                    }}
                  >
                    {finalizando ? (
                      <span
